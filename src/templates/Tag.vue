  ...
  <Layout>
    <main class="contain mt-14 mb-14">
      ...
      <article v-for="post in $page.tag.belongsTo.edges" :key="post.node.id">
        <g-link class="flex md:flex-row flex-col md:items-center items-start md:gap-10 gap-0 md:pb-3 pb-7" :to="post.node.path">
          <p class="text-dimGreyAlt">{{ post.node.date }}</p>
          <h2 class="underline underline-offset-4">{{ post.node.title }}</h2>
        </g-link>
      </article>
    </main>
  </Layout>
